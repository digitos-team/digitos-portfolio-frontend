...
const PathologyMessages = () => {
    ...
    const fetchMessages = async () => {
        try {
            setLoading(true);
            const response = await clientMessageAPI.getPathologyMessages();
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this pathology inquiry?')) {
            try {
                await clientMessageAPI.deleteClientMessage(id);
                toast.success('Pathology inquiry deleted successfully');
        ...
        <div className="p-8 max-w-7xl mx-auto">
            <div className="mb-8">
                <div className="flex items-center gap-3 mb-2">
                    <Stethoscope size={32} className="text-yellow-500" />
                    <h1 className="text-3xl font-bold font-display text-primary">Pathology Inquiries</h1>
                </div>
                <p className="text-gray-500">View and manage pathology product demo requests</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {messages.length === 0 ? (
                    <div className="col-span-full text-center py-16 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                        ...
                        <p className="text-gray-500 font-medium">No pathology inquiries found</p>
                        <p className="text-gray-400 text-sm mt-1">Demo requests will appear here</p>
                    </div>
                ) : (
                    messages.map((msg) => (
                        <div
                            ...
                            className={`
                                ...
                                ${!msg.isRead ? 'border-yellow-500/50 shadow-md shadow-yellow-500/5' : 'border-gray-100 shadow-sm'}
                            `}
                            onClick={() => setSelectedMessage(msg)}
                        >
                            {!msg.isRead && (
                                <div className="absolute top-0 right-0 w-16 h-16 pointer-events-none overflow-hidden">
                                    <div className="absolute top-0 right-0 transform translate-x-1/2 -translate-y-1/2 rotate-45 bg-yellow-500 text-black text-[10px] font-bold py-1 w-24 text-center">
                                        NEW
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="flex items-start justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    <div className={`h-10 w-10 rounded-full flex items-center justify-center text-lg font-bold ${!msg.isRead ? 'bg-yellow-500 text-black' : 'bg-gray-100 text-gray-500'}`}>
                    ...
                    >
                        <div className="p-8">
                            <div className="flex items-start justify-between mb-8">
                                <div>
                                    <div className="flex items-center gap-2 mb-2">
                                        <Stethoscope size={24} className="text-yellow-500" />
                                        <h2 className="text-2xl font-bold font-display text-primary">Pathology Inquiry</h2>
                                    </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                ...
                                <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                                    <label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1 block">Lab/Business Info</label>
                                    <div className="space-y-3 mt-3">
                                        {selectedMessage.company ? (
                                            ...
                                            <span className="text-gray-400 italic text-sm">No lab/business information provided</span>
                                        )}
                                    </div>
                                </div>
                            </div>
...
export default PathologyMessages;
